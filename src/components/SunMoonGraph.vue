...
function updateChart() {
  ...
  const baseAnnotations = {
    blueHourMorning: createBoxAnnotation(
      times.nauticalDawn,
      times.sunrise,
      'rgba(30, 144, 255, 0.2)',
    ),
    goldenHourMorning: createBoxAnnotation(
      times.sunrise,
      times.goldenHourEnd,
      'rgba(255, 215, 0, 0.2)',
    ),
    goldenHourEvening: createBoxAnnotation(
      times.goldenHour,
      times.sunset,
      'rgba(255, 215, 0, 0.2)',
    ),
    blueHourEvening: createBoxAnnotation(
      times.sunset,
      times.nauticalDusk,
      'rgba(30, 144, 255, 0.2)',
    ),
...
function createBoxAnnotation(start, end, color) {
  ...
  if (
    startIndex === -1 ||
    endIndex === -1 ||
    !chartData.value.labels[Math.min(startIndex, endIndex)] ||
    !chartData.value.labels[Math.max(startIndex, endIndex)]
  )
    return {}
  return {
    type: 'box',
...
const getIconPosition = (index, yData) => {
  if (
    ...
    !yData ||
    yData.length === 0 ||
    !smallChartOptions.value.scales
  )
    return { display: 'none' }
...
const currentPosition = computed(() =>
  getIconPosition(currentIndex.value, chartData.value.datasets[0].data),
)
